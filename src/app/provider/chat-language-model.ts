...
export class ChatLanguageModel implements LanguageModelV1 {
    ...
    private getArgs(
        { mode, prompt, maxTokens, temperature, topP, frequencyPenalty,presencePenalty, seed }: Parameters<LanguageModelV1['doGenerate']>[0]
    ) {
        ...
        switch (type) {
            case 'regular':
                return { args: { ...baseArgs, ...prepareToolsAndToolChoice(mode) }, warnings };
            case 'object-json':
                return { args: { ...baseArgs, response_format: { type: 'json_object' } }, warnings };
            case 'object-tool':
                return { 
                    args: { ...baseArgs, tool_choice: 'any', tools: [{ type: 'function', function: mode.tool }]},
                    warnings,
                };
            case 'object-grammar':
                throw new UnsupportedFunctionalityError({ functionality: 'object-grammar mode' });
            default:
...
const chatChunkSchema = z.object({
    ...
    usage: z.object({ prompt_tokens: z.number(), completion_tokens: z.number() }).optional().nullable()
});

function prepareToolsAndToolChoice(mode: Parameters<LanguageModelV1['doGenerate']>[0]['mode'] & { type: 'regular'}) {
